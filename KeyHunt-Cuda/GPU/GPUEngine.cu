...
{
public:
        explicit NvtxRange(const char* message, uint32_t color = 0xFF2563EB)
                : id_(0)
        {
                if (message == nullptr || message[0] == '\0') {
                        message = "KeyHunt Kernel";
                }
...
#define GPUENGINE_HAS_CG_PIPELINE 1
#elif __has_include(<cuda/pipeline>)
#include <cuda/pipeline>
#define GPUENGINE_HAS_CUDA_PIPELINE 1
#endif
...
int RecommendOccupancyBlockSize(int deviceId)
{
        ...
        cudaError_t deviceStatus = cudaGetDevice(&previousDevice);
        ...
        if (previousDevice != deviceId) {
                deviceStatus = cudaSetDevice(deviceId);
                if (deviceStatus != cudaSuccess) {
                        cudaGetLastError();
                        cachedBlockSize = 0;
                        if (previousDevice >= 0) {
        ...
        for (size_t i = 0; i < (sizeof(kernels) / sizeof(kernels[0])); ++i) {
                const void* kernel = kernels[i];
                if (kernel == nullptr) {
                        continue;
                }
                int minGridSize = 0;
                int blockSize = 0;
                deviceStatus = cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, kernel, 0, 0);
                if (deviceStatus == cudaSuccess) {
                ...
                if (deviceStatus == cudaErrorNotSupported || deviceStatus == cudaErrorInvalidValue) {
                        cudaGetLastError();
                        continue;
                }
                CheckCuda(deviceStatus, "cudaOccupancyMaxPotentialBlockSize", __FILE__, __LINE__);
                // Should not reach here, but reset to avoid uninitialised use if it does.
                occupancyComputed = false;
        }

        if (previousDevice != deviceId && previousDevice >= 0) {
                cudaSetDevice(previousDevice);
        }
...
__device__ void RunKeyComputation(uint64_t* keys, int stepMultiplier, ComputeFunctor compute)
{
...
#if defined(GPUENGINE_HAS_CUDA_PIPELINE)
        __shared__ cuda::pipeline_shared_state<cuda::thread_scope_block, 1> tablePipelineState;
        auto tablePipeline = cuda::make_pipeline(block, &tablePipelineState);
        bool tablePipelineActive = false;
        (void)tablePipeline;
        (void)tablePipelineActive;
#elif defined(GPUENGINE_HAS_CG_PIPELINE)
        auto tablePipeline = cg::make_pipeline(block);
        bool tablePipelineActive = false;
        (void)tablePipeline;
        (void)tablePipelineActive;
#else
...
#if defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 800) && (defined(GPUENGINE_HAS_CUDA_PIPELINE) || defined(GPUENGINE_HAS_CG_PIPELINE))
        ...
        for (int iteration = 0; iteration < stepMultiplier; ++iteration) {
                ...
                compute(tables, xPtr, yPtr, baseOffset);
        }
}
